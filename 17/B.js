...
const deltaNeighbors = [
	...new Array(7).fill(0).map((_, i) => [i+4, 0, 0]),
	...new Array(7).fill(0).map((_, i) => [-i-4, 0, 1]),
	...new Array(7).fill(0).map((_, i) => [0, i+4, 2]),
	...new Array(7).fill(0).map((_, i) => [0, -i-4, 3]),
];

function dist(from, to) {
	function neigbors([x, y, d]) {
		return deltaNeighbors
			.map(([dx, dy, nd]) => [x+dx, y+dy, nd])
	...
	console.log(`[${new Date().toISOString()}] queue: ${queue.length}`);
	...
	while (queue.length > 0) {
		...
		if (lastStatus + 5000 < Date.now()) {
			console.log(`[${new Date().toISOString()}] queue: ${queue.length}, processed: ${lastQueue - queue.length} `
				+ `(approx ${((lastQueue - queue.length) / 5).toFixed(1)}/s or ${(5000/(lastQueue - queue.length)).toFixed(3)}ms/item) `
				+ `estimated finish: ${new Date(Date.now() + queue.length / (lastQueue - queue.length) * 5000).toISOString()}`);
			lastStatus = Date.now();
