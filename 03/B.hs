...
isPotentialGear :: Char -> Bool
isPotentialGear '*' = True
isPotentialGear _ = False
...
findAllSymbols d =
    concat [
        (map (\(x, c) -> (x, y)) (filter (\(_, c) -> isPotentialGear c) (zip [0..] s :: [(Int, Char)])))
...
main = do
    ...
    let potentialGears = (map (\p -> findNumericNeighbors contents p) (findAllSymbols contents))
    let foundNumbers = filter (\n -> length n == 2) (map (\pg -> deduplicatePairListList (map (\p -> getNumberPos contents p) pg)) potentialGears)
    let gears = map (\g -> map (\l -> read (map (\p -> sybmolAtPos contents p) l) :: Int) g) foundNumbers
    let ratios = map (\(a:b:[]) -> a*b) gears
    print $ sum ratios
